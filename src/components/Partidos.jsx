...
export default function Partidos({ partidos }) {
  ...
  async function adicionar(e) {
    ...
    const { error } = await supabase.from("partidos").insert({
      sigla: sig, nome: nome.trim() || sig, espectro: esp,
    });
    setSalvando(false);
  ...
  function iniciarEdicao(p) {
    setEditId(p.id); setEditEsp(p.espectro); setEditNome(p.nome);
  }
  ...
  return (
    <div className="view-pad">
      <h1 className="page-title">Partidos e espectro</h1>
      <p className="page-sub">Gerencie a lista de partidos usada para preencher o espectro automaticamente nos lançamentos.</p>
